...
class BoardList(UserControl):
    ...
    def build(self):
        ...
        self.end_indicator = Container(
            bgcolor=colors.BLACK26, border_radius=border_radius.all(30), height=3, width=200, opacity=0.0
        )
        self.edit_field = Row(
            [
                TextField(value=self.title, width=150, height=40, content_padding=padding.only(left=10, bottom=10)),
                TextButton(text="Save", on_click=self.save_title),
            ]
        )
        self.header = Row(
            controls=[
                Text(value=self.title, style="titleMedium", text_align="left", overflow="clip", expand=True),
                Container(
                    PopupMenuButton(
                        items=[
                            PopupMenuItem(
                                content=Text(value="Edit", style="labelMedium", text_align="center", color=self.color),
                                on_click=self.edit_title,
                            ),
                            PopupMenuItem(),
                            PopupMenuItem(
                                content=Text(
                                    value="Delete", style="labelMedium", text_align="center", color=self.color
                                ),
                            ...
                            PopupMenuItem(
                                content=Text(
                                    value="Move List", style="labelMedium", text_align="center", color=self.color
                                )
                            ),
                        ],
                    ),
        ...
        self.inner_list = Container(
            content=Column(
                [
                    ...
                    self.new_item_field,
                    TextButton(
                        content=Row([Icon(icons.ADD), Text("add card", color=colors.BLACK38)], tight=True),
    ...
    def save_title(self, e):
        ...
        self.header.controls[0] = Text(
            value=self.title, style="titleMedium", text_align="left", overflow="clip", expand=True
        )
