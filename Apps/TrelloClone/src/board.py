    ...
    def create_list(self, e):
        ...
        def close_dlg(e):
            if (hasattr(e.control, "text") and not e.control.text == "Cancel") or (
                ...
                new_list = BoardList(self, self.store, dialog_text.value, color=color_options.data)
        ...
        dialog_text = TextField(label="New List Name", on_submit=close_dlg, on_change=textfield_change)
        create_button = ElevatedButton(text="Create", bgcolor=colors.BLUE_200, on_click=close_dlg, disabled=True)
        dialog = AlertDialog(
            title=Text("Name your new list"),
            content=Column(
                [
                    Container(content=dialog_text, padding=padding.symmetric(horizontal=5)),
                    color_options,
                    Row([ElevatedButton(text="Cancel", on_click=close_dlg), create_button], alignment="spaceBetween"),
                ],
                tight=True,
                alignment="center",
            ),
